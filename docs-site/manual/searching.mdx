...
`prop:r<0.9`\
cards with FSRS retrievability less than 0.9.

The following searches are available on desktop. When entered in the Browser,
Anki prepares current RWKV scores before returning the matching cards:

`prop:rwkv:r<0.9`\
cards with RWKV-Instant retrievability less than 0.9.

`prop:rwkv-curve:r<0.9`\
cards with current RWKV-Curve retrievability less than 0.9.
