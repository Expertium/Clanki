...
Each preset can limit the same-day reviews of a card
(`max_same_day_reviews`, spec `sched.max-same-day-reviews`). Once a card has
had that many reviews after its first one of the day, answering it with FSRS
active schedules review states directly instead of writing
learning/relearning queue states, skipping configured steps and FSRS sub-day
intervals. The limit applies only to presets without learning steps
(`DeckConfig::effective_max_same_day_reviews`). The old collection-wide bypass
(`BoolKey::FsrsLearningQueuesDisabled`) is migrated to a limit of 0 on every
preset when the collection opens.
...
When FSRS review order is ascending or descending retrievability, due review,
...
the card's current FSRS retrievability from its selected deck preset. Relative
Overdueness uses the same shared gather path, with the model-independent key
`-elapsed_days / interval_at_retrievability(full_state, desired_retention)`.
New cards are not included in these FSRS orderings because they do not have FSRS
retrievability yet. Equal keys use the same `fnvhash(id, mod)` tiebreaker as the
legacy SQL order. In desktop RWKV
...
with each reconstructed state's internal slow stability, fast stability, and
difficulty. The
...
parameter array. Their scalar `stability` signatures predate FSRS-7; for a
34-parameter model they explicitly assume difficulty 5 and equal slow/fast
stability. They remain compatibility approximations for add-ons, not the source
of built-in FSRS-7 metrics. An exact future public API must accept or resolve the
...
For FSRS-7, built-in callers pass internal slow stability, fast stability, and
difficulty to the native forgetting-curve mixture; they do not use the scalar
add-on compatibility helper or the stored scalar decay.
...
stored scalar decay from `card.data` for legacy expressions. Built-in FSRS
retrievability and Relative Overdueness paths bypass them and sort/filter using
preset-aware Rust metrics.
...
- Add-on/custom SQL expressions that call the sqlite FSRS helpers are still
  scalar-decay-based and can diverge (especially with FSRS-7).
...
- Exact model-based ordering in Rust is used for:
    ...
    - Review queue Relative Overdueness
    ...
    - Filtered deck Relative Overdueness (FSRS full-state target interval)
...
- Add-on helper APIs expose scalar-compatible interval-at-target-retrievability
  math:
    ...
    - `fsrs_interval_at_retrievability_by_config_batch([{request_index, config_id, stability}, ...], target_retrievability)`
      These signatures assume difficulty 5 and equal slow/fast stability for
      FSRS-7, so they are approximations rather than exact full-state APIs. The
      card batch helper resolves card presets with the batch preset resolver
