...
sync-conflict = Only one copy of Clanki can sync to your account at once. Please wait a few minutes, then try again.
sync-server-error = AnkiWeb encountered a problem. Please try again in a few minutes.
sync-client-too-old = Your Clanki version is too old. Please update to the latest version to continue syncing.
...
sync-must-wait-for-end = Clanki is currently syncing. Please wait for the sync to complete, then try again.
...
sync-conflict-explanation =
    ...
    If you choose download, Clanki will fetch the collection from AnkiWeb, and any changes you have made on this device since the last sync will be lost.
    
    If you choose upload, Clanki will send this device's data to AnkiWeb, and any changes that are waiting on AnkiWeb will be lost.
...
# Title of the sync progress window, while a collection sync runs.
sync-syncing-with-ankiweb = Syncing with AnkiWeb
# First label of the sync progress window, before the server has answered and
# there is anything to report. It replaces a plain "Checking...", which did
# not say who was being waited for.
sync-contacting-ankiweb = Contacting AnkiWeb...
